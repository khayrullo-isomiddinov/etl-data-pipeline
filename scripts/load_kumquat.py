...
KUMQUAT_CSV = BASE / "raw_data" / "kumquat_output.csv"
...
def main():
    ...
    _, rows = read_csv_smart(str(KUMQUAT_CSV))
    cur = con.cursor()
    ...
    con.commit()
    con.close()
    print(f"Kumquat delta-load complete. +{len(new_imgs)} images, +{len(new_lbls)} labels inserted.")
